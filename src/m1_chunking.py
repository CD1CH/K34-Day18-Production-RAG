...
# Global cache for semantic model
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for parent in parents:
        ...
        if not p_sentences:
            p_sentences = [parent.text]

        curr_child_text = ""
        for s in p_sentences:
            if len(curr_child_text) + len(s) > child_size and curr_child_text:
                children.append(Chunk(
                    text=curr_child_text.strip(),
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    for tok in tokens:
        ...
        if not tok_stripped:
            continue
        if re.match(r'^#{1,3}\s+', tok_stripped):
            if current_body.strip() or current_header:
                ...
                if full_chunk_text:
                    chunks.append(Chunk(
                        ...
                        metadata={**metadata, "section": current_header or "Root", "strategy": "structure", "chunk_index": len(chunks)}
                    ))
            current_header = tok_stripped
            current_body = ""
        else:
            current_body += tok + "\n\n"
